...
function charge(player) {
    var attack = random(0, 1);
    var critiq = random(0, 3);
    if (player == "PlayerOne") {
...
function finishHim(player) {
    var attack = random(0, 4);
    var critiq = random(0, 5);
    if (player == "PlayerOne") {
        if (attack == 0 && critiq == 0) {
            pvTwo -= 75;
...
while (pvOne > 0 && pvTwo > 0) {
    console.log("\nPV de", playerOne, pvOne, "PV de", playerTwo, pvTwo, "\n");
